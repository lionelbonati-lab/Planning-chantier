...
function construireOccurrencesSerie(champs, dates, serieId, existantes, existantesAssignations, estAbsence) {
  ...
  if (champs.type === "tache") {
    var personneId = champs.cible_personne_id, demi = champs.cible_demi;
    dates.forEach(function (iso) {
      // Toujours AJOUTÉE, jamais de remplacement ni de déduplication — une
      // série vient s'ajouter au contenu déjà présent ce jour-là, comme
      // l'ancien ecrireOccurrenceSerie_ (push() en fin de case). `ordre`
      // place la nouvelle tâche après celles déjà présentes sur ce
      // (personne, date, demi), pour un affichage stable.
    ...
    dates.forEach(function (iso) {
      ...
      ops.push({ type: "insert", table: "jalons", date: iso, texte: champs.texte, serie_id: serieId });
      posees++;
    });
  } else {
    ...
    dates.forEach(function (iso) {
      ops.push({ type: "insert", table: "notes", date: iso, texte: champs.texte, important: champs.important, serie_id: serieId });
      posees++;
    });
  }
