...
- **Case interdite** : orange (dépôt refusé par un glisser, point 49), bleu (dépôt permis) pendant un glisser — coloriage restauré au point 79, perdu par erreur lors de la fusion du 21e message ; bleu aussi réservé à la sélection rapide (point 45-47), vert à l'ajout.
...
- **Flèches flottantes Annuler/Refaire (points 75, 80)** : toujours visibles en haut à gauche du planning, À LA PLACE du titre "Semaine …" (qui se décale à droite pour leur laisser la place), grisées quand rien à annuler/refaire, ancrées comme la barre du bas (collées en haut de l'écran si on défile plus bas). Équivalent visuel/tactile de Ctrl+Z/Ctrl+Y.
...
Attendre le retour de Lionel sur cette nouvelle version (bulle multi-jours qui garde sa taille pendant le glisser tactile ; coloriage bleu/orange restauré pendant un déplacement ; flèches Annuler/Refaire à la place du titre "Semaine …", remontées pour ne plus chevaucher la grille) avant de commencer le travail sur la copie du vrai fichier (V3). Rien à faire côté Index.html / WebApp.gs / Planning_Format.gs tant que les interactions ne sont pas validées.
