...
Attendre le retour de Lionel sur cette nouvelle version (flèches Annuler/Refaire qui ne sont plus coupées en haut lors du défilement ; glisser d'une bulle multi-jours qui respecte désormais le point d'empoignade exact ; surlignage bleu/orange couvrant toute la largeur de la bulle pendant un glisser ; fenêtre d'édition élargie pour contenir le bouton Enregistrer, champ texte agrandi) avant de commencer le travail sur la copie du vrai fichier (V3). Rien à faire côté Index.html / WebApp.gs / Planning_Format.gs tant que les interactions ne sont pas validées.
