...
(async () => {
  ...
  // --- 1 à 6. Ordinateur : onglet, enregistrement, impression ---
  {
    ...
    verifier(blocs.actif && blocs.legendes.join() === 'Page,Marges,Colonnes,Dates,Espacements,En-tête,Pied de page,Date d’impression', 'onglet : Page, Marges, Colonnes (suite 44), Dates (suite 45), Espacements, En-tête, Pied de page, Date d’impression (suite 46) (' + blocs.legendes.join() + ')');
    ...
    verifier(defauts === 'orientation=paysage taille=normale marges.haut=12 marges.bas=12 marges.gauche=12 marges.droite=12 colonnes.jours=dynamique colonnes.largeurJour=48 colonnes.noms=dynamique colonnes.largeurNoms=30 dates.jour=abrege dates.mois=masque dates.annee=false espaces.personnes=1.6 espaces.sections=3.7 espaces.cases=1.3 espaces.legende=3.7 entete.texte= entete.chantier=true entete.date=false pied.page=true pied.date=true pied.texte= dateImpr.format=chiffres dateImpr.heure=true dateImpr.prefixe=true',
