...
(async () => {
  ...
  // --- 1 à 6. Ordinateur : onglet, enregistrement, impression ---
  {
    ...
    verifier(blocs.actif && blocs.legendes.join() === 'Page,Marges,Colonnes,Espacements,En-tête,Pied de page', 'onglet : Page, Marges, Colonnes (suite 44), Espacements, En-tête, Pied de page (' + blocs.legendes.join() + ')');
    ...
    verifier(defauts === 'orientation=paysage taille=normale marges.haut=12 marges.bas=12 marges.gauche=12 marges.droite=12 colonnes.jours=dynamique colonnes.largeurJour=48 colonnes.noms=dynamique colonnes.largeurNoms=30 espaces.personnes=1.6 espaces.sections=3.7 espaces.cases=1.3 espaces.legende=3.7 entete.texte= entete.chantier=true entete.date=false pied.page=true pied.date=true pied.texte=',
