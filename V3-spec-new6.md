...
Attendre le retour de Lionel sur cette nouvelle version (fenêtre d'édition élargie pour contenir le bouton Enregistrer, champ texte agrandi ; ajout rapide Congé/Vacances dans le menu Ajouter ; appui long requis au tactile pour ouvrir ce menu) avant de commencer le travail sur la copie du vrai fichier (V3). Rien à faire côté Index.html / WebApp.gs / Planning_Format.gs tant que les interactions ne sont pas validées.
