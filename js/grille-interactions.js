"use strict";
  ...
  // plusProcheRepereJour_(scroller) — round du 23.09.2026 (suite ×12). Trouve
  // le scrollLeft du repère .snap-jour le plus proche de la position actuelle
  // (mêmes repères que le scroll-snap CSS "1 jour" mobile, cf. leur
  // commentaire dans grille-rendu.js/style.css) — null si aucun repère
  // n'existe (hors mode "1 jour", où .snap-jour n'est simplement jamais posé :
  // cf. son site d'appel plus bas, creerDefilementManuel, qui reste alors
  // inerte sur ce point sans condition supplémentaire à vérifier ici).
  function plusProcheRepereJour_(scroller) {
    var reperes = scroller.querySelectorAll(".snap-jour");
  ...
  // creerDefilementManuel(scroller) — round du 23.09.2026 (suite ×12).
  // Lionel : « le scroll et le swipe ne fonctionnent pas sur la partie
  // tableau, elle ne fonctionne que sur la partie ou il y a les noms. »
  // Cause : cablerAjoutCellule/cablerPoigneeRedim/onPointerDownGroupeSelection
  // (plus bas) posent touch-action:none sur les cases/bulles/poignées —
  // nécessaire pour garder la main sur le geste tactile (tap = ajout, appui
  // long = sélection/redimensionnement, glissé = défilement) plutôt que de
  // laisser le navigateur trancher seul — ce qui désactive AUSSI son
  // défilement natif pour ces zones, remplacé ici par un défilement manuel
  // (scroller.scrollLeft -= dx à chaque pointermove, cf. le "if (enDefilement)"
  // des 3 sites d'appel). Seule la colonne des noms (.lbl, sans touch-
  // action:none) gardait le vrai défilement natif du navigateur — SON
  // inertie/décélération au lâcher du doigt, qu'un simple "scrollLeft -= dx"
  // par événement ne reproduisait jamais (le défilement s'arrête NET dès que
  // le doigt se lève). Vérifié : le défilement manuel fonctionnait déjà au
  // sens strict pendant le geste (cf. test_scroll_tableau.js, scrollLeft
  // bouge bien) — c'est cette absence d'inertie, comparée au glissé fluide
  // et prolongé de la colonne des noms juste à côté, qui donnait l'impression
  // que "ça ne marche pas" sur le reste du tableau.
  // Fix : mesurer la vitesse réelle du geste (moyenne mobile légère sur
  // dx/dt, pas juste le dernier échantillon — un doigt qui ralentit pile
  // avant le lâcher, très courant, donnerait sinon une vitesse quasi nulle
  // malgré un geste global rapide) et, au relâchement, prolonger le
  // défilement en décélérant (friction exponentielle Math.pow(f, dt),
  // indépendante du framerate) plutôt que de s'arrêter net — même sensation
  // qu'un défilement natif, sur les 2 axes (horizontal ET vertical, ce
  // dernier via window.scrollBy comme déjà fait pendant le geste). En mode
  // "1 jour" mobile (scroll-snap CSS actif, cf. .snap-jour), la décélération
  // se termine en calant EXACTEMENT sur le repère de jour le plus proche
  // (plusProcheRepereJour_ ci-dessus) plutôt que de s'éteindre à une position
  ...
  function creerDefilementManuel(scroller) {
    ...
    function tick(t, derniereFrame) {
      ...
      raf = requestAnimationFrame(function (t2) { tick(t2, t); });
    }
    return {
  ...
  function cablerPoigneeRedim(handleEl, bulleDom, it, cote) {
    handleEl.addEventListener("pointerdown", function (e) {
      ...
      // commentaire dans creerDefilementManuel plus haut — donne au
      ...
      function detacher() {
        ...
        // relacher() (round du 23.09.2026, suite ×12) : lance l'inertie si un
        // défilement manuel était en cours — cf. creerDefilementManuel plus
  ...
  function cablerAjoutCellule(cell) {
    ...
    cell.addEventListener("pointerdown", function (e) {
      ...
      // defilementManuel (round du 23.09.2026, suite ×12) : cf. son
      // commentaire dans creerDefilementManuel plus haut — même inertie au
      // relâchement que pour la poignée/la bulle entière, pour le glisser
  ...
  function demarrerDefilementOuSortieSelection(e, cell) {
    ...
    // mode sélection) est un défilement manuel comme les autres, avec le
    // même manque d'inertie à corriger.
    var defilementManuel = creerDefilementManuel(scroller);
    var bouge = false;
    function onMove(e2) {
      if (e2.pointerId !== pointerId) return;
  ...
  function majSemaineAffichage() {
    ...
    // état par aria-pressed en plus de sa teinte. Le calendrier de la
    // barre (.btn-calendrier) s'ouvre sur le jour affiché : valeur tenue à
    // jour à chaque rendu et à l'arrêt d'un swipe (cf. defilementArrete,
    // grille-rendu.js), pas seulement à l'appui — selon le téléphone, le
    // calendrier peut s'ouvrir avant le "click". (Suite 17 : "‹ Sem. N ›"
    // ne quitte plus le menu ⋮ en vue "1 jour" — plus de .mode-jour.)
